...
public class RouterApp extends JPanel {
    ...
    private final String fileName = FileManager.getFilePath("Роутер.txt");
	...
	public RouterApp() {
		...
		/*-------------------- */
		//Данный код демонстрирует сущность объекта `RouterApp`, который является потомком `JPanel`
		...
		showHistoryButton.addActionListener(_ -> {
			try {
				...
				if (history.isEmpty()) {
					...
					dialog.setTitle("История (Роутер) - Редактирование");
